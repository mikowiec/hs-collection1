...
monitor (ps,bus) raw_name f = do
  ...
  case typeRepArgs (typeOf f) of
   [argsType, res] | res == typeOf (undefined :: IO ()) -> return ()
   _ -> fail (printf "Invalid type for signal handler %s" (show (typeOf f)))
...
signal (ps,bus) raw_name val = do
  ...
  case Map.lookup name map of
   Just ds -> do
    printf "Signal: %s ! [%s] (monitors: %d)\n" name (take_some $ show val) (length ds)
    mapM_ (\(_,d) -> do
